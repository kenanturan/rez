...
class ReservedUsersListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    // Yardımcı sınıfları örnekleyin
    private let designHelper = ReservedUIDesignHelper()
    private let dataFetcher = ReservedUsersDataFetcher()
    ...
    private func fetchUserEmails() {
        // Data fetcher kullanarak veri alın
        dataFetcher.fetchUserEmails(forUserIDs: self.userIDs) { (emails) in
    ...
    private func setupUI() {
        ...
        // Design helper kullanarak UI öğelerini oluşturun
        tableView = designHelper.setupUserTableView(bounds: self.view.bounds)
        tableView.delegate = self
        tableView.dataSource = self
        view.addSubview(tableView)
    }
